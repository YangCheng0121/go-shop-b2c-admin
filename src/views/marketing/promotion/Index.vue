...
export default {
  ...
  methods: {
     ...
     */
    toEditPage(id) {
      this.$router.push({ name: 'PromotionEdit', params: { id: id } })
    },
    _getPromotionList() {
      ...
      this.$store.dispatch('GetPromotionList', { query, sortby, order, pageNumber, pageSize }).then((data) => {
        this.pages = data
        this.loading = false
      })
    }
