...
export default {
  ...
  computed: {
    ...
    productCategoryName() {
      ...
      if (this.productCategoryList.length) {
        return this.productCategoryList[0].name
      }
      return ''
    },
    ...mapGetters([
  ...
  data() {
    return {
      form: this.$form.createForm(this, { name: 'SpecificationAdd' }),
      formLayout: {
        labelCol: { span: 3 },
        wrapperCol: { span: 8 }
      },
  ...
  methods: {
    ...
    handleConfirm() {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this._addSpecification({ name, productCategoryId, orders, options })
        }
      })
    },
    /**
     ...
     * @private
     */
    _addSpecification({ name, productCategoryId, orders, options }) {
      addSpecification({ name, productCategoryId, orders, options }).then((res) => {
