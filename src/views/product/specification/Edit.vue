...
import { updateSpecification, getSpecification } from '@/api/specification'
...
export default {
  ...
  data() {
    return {
      form: this.$form.createForm(this, { name: 'SpecificationEdit' }),
      formLayout: {
        labelCol: { span: 3 },
        wrapperCol: { span: 8 }
      },
  ...
  methods: {
    ...
    handleConfirm() {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this._updateSpecification({ id, name, productCategoryId, orders, options })
        }
      })
    },
    /**
     ...
     * @private
     */
    _updateSpecification({ id, name, productCategoryId, orders, options }) {
      updateSpecification({ id, name, productCategoryId, orders, options }).then((res) => {
        if (res.err_code === ERR_OK) {
          this.$router.back()
        } else {
