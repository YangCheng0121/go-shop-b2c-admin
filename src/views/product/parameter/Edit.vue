...
export default {
  ...
  methods: {
    ...
    handleConfirm() {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          names = names.join(',')
          this._updateParameter({ id, productCategoryId, group, orders, names })
        }
      })
    },
    /**
     ...
     * @private
     */
    _updateParameter({ id, productCategoryId, group, orders, names }) {
      updateParameter({ id, productCategoryId, group, orders, names }).then((res) => {
    ...
    _getParameter() {
      ...
      getParameter(id).then((res) => {
        if (res.err_code === ERR_OK) {
          ...
          if (data.parameter) {
            this.names = data.parameter.map((item) => {
              return {
                id: uuidv4(),
                name: item.name
              }
