<template>
  <box>
    <a-button-group size="small" slot="left">
      ...
      <a-button icon="close" :disabled="isCanDelete" @click="_deleteDeliveryCorp">删除</a-button>
      <reload/>
      <a-dropdown>
...
import { deleteDeliveryCorp } from '@/api/delivery_corp'
...
export default {
  ...
  methods: {
     ...
     */
    toAddPage() {
      this.$router.push({ name: 'DeliveryCorpAdd' })
    },
    /**
     * 跳转编辑页面
     */
    toEditPage(id) {
      this.$router.push({ name: 'DeliveryCorpEdit', params: { id: id } })
    },
    /**
     * 获取物流公司
     * @private
     */
    _getDeliveryCorpList() {
      ...
      this.loading = true
      this.$store.dispatch('GetDeliveryCorps', { query, sortby, order, pageNumber, pageSize }).then((pageList) => {
        this.pages = pageList
        this.loading = false
      })
    },
    ...
    _deleteDeliveryCorp() {
      this.$confirm({
        ...
        onOk: () => {
          ...
          deleteDeliveryCorp(ids).then((res) => {
