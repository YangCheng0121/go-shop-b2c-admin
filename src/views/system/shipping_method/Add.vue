      ...
      <a-form
        ...
        <a-form-item label="支持支付方式">
          <a-checkbox-group v-decorator="['payment_shipping_methods']">
            <a-checkbox v-for="method in paymentMethods" :key="method.id" :value="method.id">
              {{ method.name }}
            </a-checkbox>
          </a-checkbox-group>
        </a-form-item>
        ...
        <a-form-item label="排序">
          <a-input v-decorator="['orders']" type="number"/>
        </a-form-item>
...
import { addShippingMethod } from '@/api/shipping_method'
...
export default {
  ...
  data() {
    return {
      form: this.$form.createForm(this, { name: 'ShippingAdd' }),
      formLayout: {
        labelCol: {
          span: 3,
          sm: 6,
  ...
  mounted() {
    this.$store.dispatch('GetDeliveryCorps')
    ...
    handleSubmit() {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.spinning = true
          addShippingMethod(JSON.stringify(shippingMethod)).then((res) => {
            this.spinning = false
