...
import { getPaymentMethod, updatePaymentMethod } from '@/api/payment_method'
...
export default {
  ...
  data() {
    return {
      form: this.$form.createForm(this, { name: 'PaymentMethodEdit' }),
      formLayout: {
        labelCol: {
          span: 3,
          sm: 6,
    ...
    handleSubmit() {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.spinning = true
          updatePaymentMethod(JSON.stringify(paymentMethod)).then((res) => {
            if (res.err_code === ERR_OK) {
              this.spinning = false
              this.$router.back()
            }
